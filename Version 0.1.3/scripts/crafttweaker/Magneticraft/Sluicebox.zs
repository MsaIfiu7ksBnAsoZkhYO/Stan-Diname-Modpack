...
mods.magneticraft.SluiceBox.addRecipe(
	...
	0.75 , <ore:layerSand>.firstItem,
	0.50 , <ore:layerSand>.firstItem,
	0.25 , <ore:layerSand>.firstItem,
	0.10 , <ore:layerSand>.firstItem,
	0.05 , <ore:layerSand>.firstItem,
	true
);
...
for Item in GlobalSecondaryOre {
    if (debug) {print("Current Sluicebox Item:	"~Item);}
	...
	if ( 5 >= HarvestLevel ) {
        ...
        for Left,Right in Recipes {
			...
			var Output2 = GlobalGimmeOreDict("dustTiny",Item);
            if ( 
                !(Input.empty)
                &
                !(Output.empty)
