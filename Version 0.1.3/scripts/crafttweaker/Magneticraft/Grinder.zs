...
for Item in GlobalHarvestLevel {
    if (debug) {print("Magneticraft Grinder Item:	"~Item);}	
    ...
    if ( 100 >= HarvestLevel ) {
        ...
        for Left,Right in Recipes {
            if (debug) {print("Crossroads Grindstone Input:	"~Left);}	
            if (debug) {print("Crossroads Grindstone Output:	"~Right);}	
