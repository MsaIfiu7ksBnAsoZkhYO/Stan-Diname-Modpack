...
mods.magneticraft.CrushingTable.addRecipe( 
	...
	//	Coal pellet to ash
mods.magneticraft.CrushingTable.addRecipe( 
    <rockhounding_oretiers:tier_items:5>, 
...
for Item in GlobalHarvestLevel {
    if (debug) {print("Magneticraft Crushing Table Item:	"~Item);}	
    ...
    if ( 5 >= HarvestLevel ) {
        ...
        for Left,Right in Recipes {
            if (debug) {print("Magneticraft Crushing Table Input:	"~Left);}
            if (debug) {print("Magneticraft Crushing Table Output:	"~Right);}		
